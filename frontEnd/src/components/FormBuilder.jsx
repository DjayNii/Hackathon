...
function FormBuilder() {
  ...
  // Add a new question
  const addQuestion = () => {
    setQuestions([...questions, { id: Date.now(), question: "", questionType: "shortAnswer", options: [""] }]);
  };

  // Update a question's data
  const updateQuestion = (id, updatedData) => {
    setQuestions((prevQuestions) => prevQuestions.map((q) => (q.id === id ? updatedData : q)));
  };
  ...
  return (
    <Container className="mt-4">
      ...
      {questions.map((q, index) => (
        <QuestionMaker key={q.id} id={q.id} data={q} updateQuestion={updateQuestion} removeQuestion={removeQuestion} />
      ))}

      <Button variant="secondary" className="mt-3" onClick={addQuestion}>+ Add</Button>
      <Button variant="primary" className="mt-3 ms-2" onClick={handleSubmit}>Submit Form</Button>
    </Container>
  );
}
